    ...
    >
      ...
      >
        ...
        <div className="text-center">
          <p className="font-extrabold text-[15px] text-[#333]">
            Keluar dari aplikasi?
          </p>
          <p className="text-[12px] text-[#777] mt-1">
            Sesi Anda akan diakhiri dan Anda perlu masuk ulang.
          </p>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-slate-100 bg-amber-50">
        <p className="text-[10px] font-extrabold text-amber-500 tracking-widest uppercase mb-1">
          ⚠ Mode Demo
        </p>
        <p className="text-[11px] text-amber-700 leading-snug">
          Fitur ini <strong>hanya untuk keperluan presentasi</strong>. Di sistem
          nyata, login dilakukan via MetaMask kembali lewat halaman sign-in —
          setiap wallet punya satu akun tetap.
        </p>
      </div>
      ...
      <div className="p-2 flex flex-col gap-0.5">
        {DEMO_ACCOUNTS.map((acc) => {
          const isActive =
            acc.wallet.toLowerCase() === currentWallet.toLowerCase();
          return (
            <button
              key={acc.id}
              onClick={() => {
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="m-0 font-black text-[13px] text-slate-800 truncate">
                  {acc.name}
                </p>
                <p className="m-0 text-[10px] text-slate-400 font-semibold truncate">
                  {acc.role}
                </p>
              </div>
              {isActive && (
                <Check
                  size={14}
                  className="text-[#588157] shrink-0"
                  strokeWidth={3}
                />
              )}
            </button>
      ...
      <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50">
        <p className="text-[10px] text-slate-400 text-center">
          Wallet address bersifat dummy — bukan akun MetaMask sungguhan
        </p>
      </div>
    </div>
  );
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="leading-tight text-left">
                <p className="m-0 font-extrabold text-[13px]">
                  {mounted ? user.name : "..."}
                </p>
                <p className="m-0 text-[10px] opacity-80 uppercase">
                  {mounted ? user.role : "..."}
                </p>
              </div>
              <ChevronDown
