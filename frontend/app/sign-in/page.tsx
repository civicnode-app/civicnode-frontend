...
export default function SignIn() {
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      provider: "google",
      options: { redirectTo: `${location.origin}/api/auth/google` },
    });
    } catch (err: any) {
...
<div style={{ display: 'flex', minHeight: '100vh', alignItems: 'center', justifyContent: 'center', backgroundColor: '#588157' }}>
      <div style={{ backgroundColor: '#DAD7CD', borderRadius: '30px', padding: '50px 40px', width: '100%', maxWidth: '400px', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
        <div style={{ textAlign: 'center', marginBottom: '35px' }}>
          <div style={{ fontSize: '28px', fontWeight: '800', display: 'flex', justifyContent: 'center', gap: '4px' }}>
            <span style={{ color: '#000' }}>CIVIC</span>
            <span style={{ color: '#588157' }}>NODE</span>
          </div>
          <p style={{ color: '#666', marginTop: '8px', fontSize: '14px' }}>Masuk ke akun Anda</p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <button
            ...
            disabled={loading}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', width: '100%', padding: '14px', borderRadius: '50px', border: '2px solid #dadce0', backgroundColor: 'white', color: '#3c4043', fontWeight: '700', fontSize: '15px', cursor: loading ? 'not-allowed' : 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', transition: 'box-shadow 0.2s' }}
          >
          ...
          <button
            ...
            disabled={loading}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', width: '100%', padding: '14px', borderRadius: '50px', border: '2px solid #e8761b', backgroundColor: 'white', color: '#e8761b', fontWeight: '700', fontSize: '15px', cursor: loading ? 'not-allowed' : 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', transition: 'box-shadow 0.2s' }}
          >
