  ...
  return (
    <div className="flex min-h-screen w-full">
      ...
      <main className="flex-1 bg-[#588157] p-8 flex flex-col gap-6">
        ...
        <header className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div ref={dropRef} className="relative">
              ...
              {dropdownOpen && (
                <div className="absolute top-[calc(100%+8px)] right-0 bg-white rounded-[18px] shadow-[0_8px_32px_rgba(0,0,0,0.18)] overflow-hidden min-w-47.5 z-100 animate-[fadeDropdown_0.15s_ease-out]">
                  ...
                  {VIEW_OPTIONS.map((opt) => (
                    ...
                    >
                      <span>{opt.label}</span>
                      <span
                        className={`text-[11px] font-semibold ${viewMode === opt.value ? "text-[#a3b18a]" : "text-[#bbb]"}`}
                      >
                        {opt.desc}
                      </span>
                    </button>
        ...
        <div className="bg-[#CADBB7] rounded-[45px] p-7 flex gap-5 flex-1 items-stretch transition-all duration-350 ease-[cubic-bezier(0.25,1,0.5,1)]">
          {visibleNodes.map((node) => (
            <div
              key={node.id}
              className="flex-1 min-w-0 bg-white rounded-[36px] h-137.5 p-7 flex flex-col justify-between shadow-[0_4px_16px_rgba(0,0,0,0.06)] transition-all duration-300 ease-[cubic-bezier(0.25,1,0.5,1)] cursor-pointer hover:scale-[1.025] hover:shadow-[0_12px_32px_rgba(0,0,0,0.14)]"
            >
              {/* Card top */}
              <div className="flex justify-between items-start">
                <div>
                  <p className="font-black text-lg m-0 text-black">
                    {node.label}
                  </p>
                  <p className="font-extrabold text-xs m-0 text-[#555] mt-1">
                    {node.id}
                  </p>
                </div>
                <p className="font-extrabold text-xs m-0 text-[#555]">
                  {node.timestamp}
                </p>
              </div>
